...
class KMSEncryptor:
    ...
    def encrypt(self, plaintext):
        ...
        try:
            ...
            # 3. Pack: base64(EncryptedDataKey)::base64(Nonce)::base64(Ciphertext)
            # We must store the EncryptedDataKey so we can decrypt later!
            ...
            return f"PLAINTEXT_ERROR::{plaintext}" # Fallback? Or raise?
...
""",
    ...
    def _get_all_nodes(self, intent=None):
        ...
        if intent:
            # Precise SQL filtering based on extracted intent
            if intent.get("brand"):
                query_parts.append("AND (metadata_->>'brand' ILIKE %s OR metadata_->>'product_name' ILIKE %s)")
                params.extend([f"%{intent['brand']}%", f"%{intent['brand']}%"])
            
            if intent.get("product") and len(intent.get("product")) > 2:
                # If product name is specific, filter by it
                query_parts.append("AND (metadata_->>'product_name' ILIKE %s OR text ILIKE %s)")
                params.extend([f"%{intent['product']}%", f"%{intent['product']}%"])
                
            if intent.get("shop") and intent.get("shop").strip():
                query_parts.append("AND metadata_->>'shop_name' ILIKE %s")
                params.append(f"%{intent['shop']}%")
            
            # Exclusion logic in SQL
            if intent.get("excluded_terms"):
        ...
        if not nodes and intent:
            # Fallback to broader search if filtering was too strict
            print("DEBUG: Filtered results empty. Falling back to broader fetch.")
            return self._get_all_nodes() # Recurse once without intent
    ...
    def _get_recent_history(self, user_id, limit=6):
        """Fetches and decrypts recent chat history for context."""
    ...
    def _rerank_results(self, user_query, nodes, user_llm):
        ...
        prompt = (
            ...
            f"YOUR TASK: Identify which candidates are 100% relevant to the user's specific product request. "
            f"Exclude products that are similar but different (e.g. if user wants Milk Powder, exclude Chocolate or Fresh Milk).\n"
            f"Output ONLY a comma-separated list of IDs that are highly relevant (e.g. '0, 2, 5'). If none, output 'NONE'."
        )
        ...
        try:
            ...
            return nodes # Fallback to original top nodes
    ...
    async def query(self, text, user_id=None):
        ...
        # Fetch history first
        history_str = self._get_recent_history(user_id, limit=6)
        
        # Rewrite query if history exists
        rewritten_query = text
        if history_str:
            rewritten_query = self._rewrite_query(text, history_str, user_llm)
        else:
            print("DEBUG: No history found, skipping rewrite.")

        # From now on, use 'rewritten_query' for search logic, but keep 'text' (original) for final response generation
        search_query = rewritten_query

        # 1. NEW: Extract Structured Search Intent for precision SQL filtering
        search_intent = self._get_search_intent(search_query, user_llm)
        ...
        top_candidates = scored_nodes[:40]
        ...
        context_list = []
        for n in final_nodes[:15]: # Take top 15 after reranking
        ...
        final_prompt = template.format(context_str=context_str, query_str=search_query, user_query=text)
        ...
        return {
            ...
            "backend_version": "v3_case_insensitive"
        }
