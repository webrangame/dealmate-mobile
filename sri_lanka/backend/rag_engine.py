...
""",
    ...
    async def log_chat(self, user_id, query, response, ip_address, metadata=None):
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO rag.chat_logs (user_id, query, response, ip_address, metadata) VALUES (%s, %s, %s, %s, %s)",
                (user_id, query, response, ip_address, json.dumps(metadata) if metadata else None)
            )
            cur.close()
            conn.close()
        except Exception as e:
    ...
    def _get_all_nodes(self):
        """Helper to get all nodes for BM25 retrieval from the 'rag' schema"""
        url = make_url(self.db_url)
        print(f"DEBUG: SYNC DB: Connecting to {url.host}...")
        conn = psycopg2.connect(
            host=url.host,
            port=url.port,
            database=url.database,
        ...
        print("DEBUG: SYNC DB: Connected. Fetching rows...")
        cur = conn.cursor()
        # Explicitly use the 'rag' schema
        # Explicitly use the 'rag' schema and honor 'is_enabled'
        cur.execute("SELECT node_id, text, metadata_, id FROM rag.data_supermarket_docs WHERE is_enabled = TRUE")
        print("DEBUG: SYNC DB: Query executed. Fetching...")
        rows = cur.fetchall()
        print(f"DEBUG: SYNC DB: Fetched {len(rows)} rows.")
        cur.close()
        conn.close()
        ...
        for row in rows:
            ...
            # Ensure id_ is a string and fallback to db_id if node_id is missing
    ...
    async def _ensure_user_key(self, user_id):
        ...
        # Try to find existing key in the database (from public.users table)
        try:
            url = make_url(self.db_url)
            ...
            # Check if user_id is integer (database id) or string (email)
            if str(user_id).isdigit():
                cur.execute("SELECT litellm_virtual_key FROM users WHERE id = %s", (user_id,))
            else:
                cur.execute("SELECT litellm_virtual_key FROM users WHERE email = %s", (user_id,))
                
            result = cur.fetchone()
            cur.close()
            conn.close()
            
            if result and result[0] and result[0].startswith("sk-"):
                db_key = result[0]
                print(f"Found existing key in database for user {user_id}: {db_key[:7]}...")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                print(f"Ensuring user exists in LiteLLM: {user_id}")
                ...
                print(f"Generating virtual key for user: {user_id} with alias {alias}")
                ...
                if key_resp.status_code in [200, 201]:
                    ...
                    if virtual_key and virtual_key.startswith("sk-"):
                        print(f"Successfully generated key for user {user_id}: {virtual_key[:7]}...")
                        ...
                        try:
                            ...
                            cur = conn.cursor()
                            if str(user_id).isdigit():
                                cur.execute("UPDATE users SET litellm_virtual_key = %s WHERE id = %s", (virtual_key, user_id))
                            else:
                                cur.execute("UPDATE users SET litellm_virtual_key = %s WHERE email = %s", (virtual_key, user_id))
                            conn.commit()
                            cur.close()
                            conn.close()
                        except:
                        ...
                        print(f"Invalid key format received for {user_id}: {virtual_key}")
                else:
                    print(f"Failed to generate key for {user_id}: {key_resp.status_code} - {key_resp.text}")
            except Exception as e:
                print(f"Error in _ensure_user_key for {user_id}: {e}")
    ...
    async def ingest_product_batch(self, products):
        """
        Ingests a list of product dictionaries into the vector store.
        Expected format:
        [
            {
                "brand": "Brand",
        ...
        print(f"Ingesting batch of {len(products)} products...")
        documents = []
        ...
        for p in products:
            ...
            text_content = (
                ...
                f"Price: {p.get('price')}\n"
                f"Size: {p.get('size')}\n"
                f"Deal: {p.get('savings')}"
            )
            
            # Metadata for filtering and frontend display
            metadata = {
                ...
                "type": "promotion", # Type flag for filtering
                "country": "LK"
            }
        ...
        print(f"Successfully ingested {len(nodes)} promotion nodes.")
        return len(nodes)
    ...
    async def delete_documents_by_shop(self, shop_name):
        ...
        try:
            ...
            cur.close()
            conn.close()
            print(f"Cleanup: Deleted {deleted_count} nodes for shop '{shop_name}' from database.")
            
            # Reload index to reflect changes
            self.index = self._load_index()
            return deleted_count
        except Exception as e:
            print(f"Error during cleanup for {shop_name}: {e}")
            return 0
    ...
    async def get_shop_items(self, shop_name):
        """Retrieves all product items for a specific shop from the database."""
        url = make_url(self.db_url)
        try:
            conn = psycopg2.connect(
                host=url.host,
            ...
            # Query all nodes for the given shop_name in metadata_
            # We want unique products based on image_url if possible, or just all of them
            query_str = """
                SELECT metadata_ 
    ...
    async def query(self, text, user_id=None):
        print(f"DEBUG: START QUERY: '{text}' for user: {user_id}")
        import sys
        sys.stdout.flush()
        if not self.index:
        ...
        # Setup transient user-specific LLM and Embed models
        user_llm = LiteLLM(
            model="openai/gemini-2.0-flash", 
            api_key=user_key,
            api_base=self.litellm_api_base,
        ...
        # Use global Settings.embed_model to ensure consistency with vector store (384 dim)
        user_embed_model = Settings.embed_model

        # GUARDRAIL 1: Input Safety Check
        is_safe = await self._check_input_safety(text, user_llm)
        if not is_safe:
        ...
        # 1. Intent Classification: Decide if this is a price query or a general conversation
        try:
            ...
            is_price_query = "yes" # Default
            
            if clean_text in greetings or len(clean_text) < 3:
                is_price_query = "no"
            else:
                classification_prompt = (
                    f"You are a router for a supermarket assistant. "
                    f"Is the following user input a request for specific product prices, availability, or comparison? "
                    f"Answer only 'yes' if it is a price/product query, or 'no' if it is a general greeting, thanks, "
                    f"or general question about how to use the app or general shopping advice.\n\n"
                    f"Input: {text}"
                )
                
                # Use the user_llm instance for consistency
                class_response = user_llm.complete(classification_prompt)
                is_price_query = str(class_response).lower().strip()
        ...
        template = (
            ...
            "{context_str}\\n"
            "---------------------\\n\\n"
            "QUESTION: Find all relevant deals for '{query_str}' across all shops. Provide as many results as possible from BOTH Coles and Woolworths.\\n\\n"
            "CRITICAL RULES:\\n"
            ...
            "2. **MATCHING**: Be lenient with sizes and exact naming. If it's the same brand/type, it's a match.\\n"
            "3. **PRICE CALCULATION**: If a price title is corrupted (e.g. '$$1166'), use 'WAS' and 'SAVE' to derive it. (E.g. WAS $32 SAVE $16 = $16 price).\\n"
            "4. **OUTPUT FORMAT**: Use this Markdown table only. Include as many rows as relevant results you found:\\n"
            "   | Product | Store | Price | Deal |\n"
            "   |---|---|---|---|\n"
            "   | [Name] | [Coles or Woolworths] | [Price] | [Deal Info] |\\n\\n"
            "5. **VERDICT**: Provide a short verdict comparing the prices. Who is cheaper? Is it a tie?\\n"
            "6. **NO DATA**: If the product is truly not in the context for ANY shop, say 'Product not found'. Do not hallucinate.\\n"
            "7. **NO CHAT**: Do not start with 'Based on the context...' or 'I found...'. Start directly with the table."
        )
        qa_template = PromptTemplate(template)

        # Enhanced Keyword Retrieval
        # Filter out generic question/filler words
        ...
        # Strip common punctuation and split
        import re
        ...
        print(f"DEBUG: TRACE: Searching for keywords: {keywords}")
        print(f"DEBUG: TRACE: Brand keywords: {brand_keywords}")
        sys.stdout.flush()

        try:
            all_nodes = self._get_all_nodes()
            print(f"DEBUG: TRACE: Fetched {len(all_nodes)} total nodes from DB.")
            sys.stdout.flush()
        except Exception as e:
        ...
        for n in all_nodes:
            ...
            for bkw in brand_keywords:
                if re.search(r'\b' + re.escape(bkw) + r'\b', txt):
                    score += 80 # Increased from 60 to 80
            ...
            # 3. Exact Sequence Match (bonus for multi-word queries)
            clean_query = " ".join(keywords).lower()
            if clean_query and clean_query in txt:
                score += 120 # Increased from 100 to 120
                    
            # 4. Penalize Noise (Reduced to -40)
            for noise in noise_keywords:
                if noise in txt:
                    score -= 40 
            
            # 5. Shop Name Mention Bonus (Increased to 60)
            for shop in ["coles", "woolworths"]:
                if shop in text.lower() and shop in n.metadata.get("shop_name", "").lower():
                    score += 60
            
            # 6. Multi-word match bonus (High priority for specific products)
            if len(keywords) >= 2 and all(re.search(r'\b' + re.escape(kw) + r'\b', txt) for kw in keywords):
                score += 200 # Increased from 150 to 200
            
            # 7. Price Evidence Bonus - prioritize nodes that actually have prices
            if "$" in txt or "price" in txt.lower() or "save" in txt.lower():
                score += 50
                
            # 8. Title/Header Bonus
            if txt.strip().startswith("#") or (len(txt) < 100 and any(kw in txt for kw in keywords)):
                score += 40
            
            if score >= 35: # Lowered threshold from 50 to 35
                scored_nodes.append(NodeWithScore(node=n, score=score))
        
        # Sort and take top nodes
        scored_nodes.sort(key=lambda x: x.score, reverse=True)
        
        # Diversity-aware Selection: Try to get representation from both shops
        coles_nodes = [n for n in scored_nodes if n.node.metadata.get("shop_name") == "Coles"]
        ww_nodes = [n for n in scored_nodes if n.node.metadata.get("shop_name") == "Woolworths"]
        

        # Take top 20 from each if possible
        coles_top = coles_nodes[:20]
        ww_top = ww_nodes[:20]
        
        print(f"DEBUG: Found {len(coles_nodes)} Coles nodes, {len(ww_nodes)} Woolworths nodes.")
        print(f"DEBUG: Coles top scores: {[n.score for n in coles_top]}")
        print(f"DEBUG: Woolworths top scores: {[n.score for n in ww_top]}")
        sys.stdout.flush()

        # Interleave them ...
        interleaved = []
        for i in range(max(len(coles_top), len(ww_top))):
            if i < len(coles_top): interleaved.append(coles_top[i])
            if i < len(ww_top): interleaved.append(ww_top[i])
            
        top_nodes = interleaved[:40]
        
        print(f"DEBUG: Interleaved top nodes count: {len(top_nodes)}")
        for n in top_nodes:
            print(f"DEBUG: Node Shop: {n.node.metadata.get('shop_name')}, Score: {n.score}")
        sys.stdout.flush()
        ...
        if not top_nodes:
            ...
            try:
                # Use user embed model for retrieval and honor 'is_enabled'
                from llama_index.core.vector_stores import MetadataFilters, MetadataFilter
                filters = MetadataFilters(filters=[MetadataFilter(key="is_enabled", value=True)])
                vector_retriever = self.index.as_retriever(
                    similarity_top_k=40,  # Increased from 20 to 40 for even more depth
                    embed_model=user_embed_model,
                    filters=filters
                )
                top_nodes = vector_retriever.retrieve(text)
            except:
        ...
        # Prepare context with explicit shop labels for the LLM
        context_list = []
        for n in top_nodes:
            shop = n.node.metadata.get("shop_name", "Unknown").upper()
            context_list.append(f"--- SOURCE: {shop} ---\n{n.node.text}")
        context_str = "\n\n".join(context_list)
        ...
        final_prompt = template.format(context_str=context_str, query_str=text)
        ...
        for node in top_nodes:
            ...
            if img_url and img_url not in seen_images and shop_name in shops_in_response:
                # Limit to e.g., 10 images per shop to allow showing multiple specific products
                if images_per_shop.get(shop_name, 0) < 10:
                    ...
                    extracted_metadata.append({
                        ...
                        "product": m.get("product_name"), # Mobile alias
                        "price": m.get("price"),
        ...
        if product_not_found:
            print(f"Product not found in PDF. Triggering fallback scraper for: {text}")
            try:
                from web_scraper import search_ddg
                online_results = search_ddg(text)
                ...
                if online_results:
                    ...
                    final_answer += "\n*Note: These prices are from a general web search and may not reflect in-store catalogue specials.*"
                else:
        ...
        if not product_not_found:
            is_faithful = await self._check_output_faithfulness(final_answer, top_nodes, user_llm)
            if not is_faithful:
                 return {
